...
### ATT&CK techniques & status at a glance

The same eleven boxes viewed by **MITRE ATT&CK coverage** — the primary technique chain each one
...
> the live flag values await a solve under my own account. The internal quality scoring (mean ≈ 8.0,
> none below 7) is in [STUB_AUDIT.md](STUB_AUDIT.md); ATT&CK coverage and the gap backlog are in
> [ATT&CK_GAP_MATRIX.md](ATT&CK_GAP_MATRIX.md).
...
*Part of the [rootdrifter](https://github.com/rootdrifter) security portfolio — full writeup at [rootdrifter.io/portfolio/gauntlet/](https://rootdrifter.io/portfolio/gauntlet/). Built and maintained by a security-cleared candidate — current UK government clearance obtained through employment in a high-security environment.*
